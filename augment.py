...
"""
#code from https://github.com/boschresearch/data-augmentation-coling2020
...
open_thesaurus = OpenThesaurusDb(host="localhost", user="root", passwd="Matos+202", db_name="database_name")
#Follow https://pypi.org/project/py-openthesaurus/ to set up the open thesaurus for synonym replacement data augmentation
        ...
        for j, token in enumerate(sentence.tokens):
            ...
            if label == "O":
                ...
                if np.random.binomial(1, replace_ratio, 1)[0]:
                    ...
                    replaced_mention = candidates[random_idx].split()
                    generated_token = Token(replaced_mention[0])
                    generated_token.set_label("gold", category)
                    generated_sentence.add_token(generated_token)
                    for t in replaced_mention[1:]:
                        generated_token = Token(t)
                        generated_token.set_label("gold", category)
    ...
    for i in range(num_generated_samples):
        ...
        for mask, token in zip(masks, sentence.tokens):
            ...
            if mask == 0 or token.text.lower() in STOPWORDS:
                ...
                synonyms = set(open_thesaurus.get_synonyms(word=token.text))
